# ============================================================
# JUPITER - SENSITIVITY TO NUMBER OF CLUSTERS
#
# Purpose:
...
# Silhouette requires pairwise distances.
# Use a fixed reproducible sample for computational efficiency.
sample_size <- 3000
...
for (K in K_values) {
  ...
  # ----------------------------------------------------------
  # Crisp partition from maximum membership
  # ----------------------------------------------------------
  ...
  # ----------------------------------------------------------
  # Silhouette on same fixed sample
  # ----------------------------------------------------------
...
# ============================================================
# 10. Normalize fuzzy metrics for comparison across K
# ============================================================
...
results <- results %>%
  mutate(
    ...
    # Normalized FPC:
    # minimum theoretical FPC = 1/K
    FPC_normalized =
      (FPC - 1 / K) /
      (1 - 1 / K),

    # Normalized PE:
    # maximum theoretical entropy = log(K)
    PE_normalized =
      PE / log(K)
  )
...
# ============================================================
# 12. Best K according to each metric
# ============================================================
...
best_fpc <- results$K[
  which.max(results$FPC_normalized)
]

best_pe <- results$K[
  which.min(results$PE_normalized)
]
...
cat(
  "Maximum normalized FPC: K =",
  best_fpc,
  "\n"
)

cat(
  "Minimum normalized PE: K =",
  best_pe,
  "\n"
)
...
# ============================================================
# 14. GRAPH 2
# Normalized fuzzy partition metrics
# ============================================================
...
write.csv(
  results,
  "jupiter_cluster_number_results.csv",
  row.names = FALSE
)
...
ggsave(
  filename = "jupiter_K_silhouette.png",
  plot = p_silhouette,
  width = 8,
  height = 5,
  dpi = 300
...
ggsave(
  filename = "jupiter_K_fuzzy_normalized.png",
  plot = p_fuzzy,
  width = 8,
  height = 5,
  dpi = 300
...
ggsave(
  filename = "jupiter_K_fuzzy_raw.png",
  plot = p_raw,
  width = 8,
  height = 5,
  dpi = 300
